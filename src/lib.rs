...
use std::env;
use teloxide::{prelude::*, utils::chat::get_chat_member};
...
pub fn run() {
    ...
    listen_to_update(telegram_token, |update| {
        if let UpdateKind::Message(msg) = update.kind {
            ...
            let chat_member = get_chat_member(target_channel, msg.from().id).send().await;

            match chat_member {
                Ok(member) => {
                    if member.status == teloxide::types::ChatMemberStatus::Member || member.status == teloxide::types::ChatMemberStatus::Administrator || member.status == teloxide::types::ChatMemberStatus::Creator {
                        ...
                        _ = tele.send_message(chat_id, "Please join the target channel to get your questions answered.".to_string());
                    }
                }
                Err(e) => {
